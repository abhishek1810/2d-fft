...
int main(int argc, char **argv){
    ...
    if ( my_rank == 0 ) {
        for(i=1;i<p;i++){
            index = i*(div*N);
            MPI_Send(&A[index], N*div, complexStruct, i, tag, MPI_COMM_WORLD);
            MPI_Send(&B[index], N*div, complexStruct, i, tag, MPI_COMM_WORLD);
        }
    } else {
        MPI_Recv(&A[0], N*div, complexStruct, 0, tag, MPI_COMM_WORLD, &status);
        MPI_Recv(&B[0], N*div, complexStruct, 0, tag, MPI_COMM_WORLD, &status);
    }
    /*------------------------------*/
    ...
    if ( my_rank == 0 ) {
        for(i=1;i<p;i++){
            index = i*(div*N);
            MPI_Recv(&A[index], N*div, complexStruct, i, tag, MPI_COMM_WORLD, &status);
            MPI_Recv(&B[index], N*div, complexStruct, i, tag, MPI_COMM_WORLD, &status);
        }
    } else {
        MPI_Send(&A[0], N*div, complexStruct, 0, tag, MPI_COMM_WORLD);
        MPI_Send(&B[0], N*div, complexStruct, 0, tag, MPI_COMM_WORLD);
    }
    /*------------------------------*/
    ...
    for (i = 0; i < div ; i++) {
        for(j = 0; j < N ; j++) {
            index = i*N+j;
            C[index].r = (A[index].r * B[index].r) - (A[index].i * B[index].i);
            C[index].i = (A[index].r * B[index].i) + (A[index].i * B[index].r);
        }
    }
    /*------------------------------*/
    ...
    if ( my_rank == 0 ) {
        for(i=1;i<p;i++){
            index = i*(div*N);
            MPI_Recv(&C[index], N*div, complexStruct, i, tag, MPI_COMM_WORLD, &status);
        }
    } else {
        MPI_Send(&C[0], N*div, complexStruct, 0, tag, MPI_COMM_WORLD);
    }
    /*------------------------------*/
    ...
    if ( my_rank == 0 ) {
        for(i=1;i<p;i++){
            index = i*(div*N);
            MPI_Send(&C[index], N*div, complexStruct, i, tag, MPI_COMM_WORLD);
        }
    } else {
        MPI_Recv(&C[0], N*div, complexStruct, 0, tag, MPI_COMM_WORLD, &status);
    }
    /*------------------------------*/
